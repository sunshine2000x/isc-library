...
static void isc_handle_user_msg(struct isc_device *idev, struct isc_msg *msg)
{
	...
	do {
		...
		if (li->handler)
			rc |= li->handler(msg->d, msg->len, li->arg);
		pos = list_next(pos);
...
static int isc_create_queue(struct isc_queue *q, uint16_t msz,
			    uint16_t num, uint8_t *mem)
{
...
static int isc_send_msg(struct isc_handle *isc, void *msg,
...
static int isc_try_bind(struct isc_device *idev, uint32_t msz,
			uint32_t num, bool is_send)
{
	...
	q->mem = mmap(0, bind.size, PROT_READ | PROT_WRITE, MAP_SHARED,
		      idev->fd, bind.mem);
	if (q->mem == MAP_FAILED)
		return -1;

	if (is_send && bind.stat == 1) {
		pthread_mutex_lock(&idev->send_lock);
		idev->send_ready = true;
		pthread_mutex_unlock(&idev->send_lock);
	}
...
static struct list *isc_create_new_listener(msg_handler handler, void *arg)
{
	struct list *pos;
	struct isc_listener *li;
			    ...
			    msg_handler handler, void *arg)
{
	...
	do {
		li = (struct isc_listener *)list_get(pos, NULL);
		if (li->handler == handler && li->arg == arg)
			goto _exit;
		pos = list_next(pos);
	} while (pos != idev->listener_list);

_null_pos:
	pos = isc_create_new_listener(handler, arg);
	if (!pos)
		goto _exit;
	     ...
	     struct isc_handle **isc)
{
	...
	if (direct == ISC_DIR_BIDIRECTION)
		flags = O_RDWR;

	fd = open(ISC_DEV_NAME, flags);
	if (fd < 0)
		return -1;
	...
	if (direct & ISC_DIR_RECV) {
		rc = isc_try_bind(idev, r->msz, r->num, false);
		if (rc < 0) {
			isc_destroy_task(idev);
			free(idev);
			close(fd);
